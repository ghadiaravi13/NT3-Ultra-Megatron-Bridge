...
# LOCAL PATCH (bind-mounted from repos/Megatron-Bridge/debug_1bb35c/):
# Enables the fine-grained MXFP8 parameter all-gather *backward* hook when the
# forward hook is enabled, fixing a cuBLAS "Pointer to A matrix data cannot be
# null" crash in the MTP `eh_proj` dgrad on Nemotron 3 Ultra. See
# repos/Megatron-Bridge/debug_1bb35c/README.md for the full writeup. Remove this
# file (and the matching --custom_mounts entry in the launch scripts) once the
# fix is upstreamed to Megatron-LM.
    ...
    def __init__(
        ...
        super().__init__(
            ...
            module=MegatronFSDP(
                ...
                # LOCAL PATCH: mirror the forward-hook condition for MXFP8 so
                # that the FP8 column-wise weight buffer is re-gathered before
                # backward. Without this, modules outside any MoE-overlap FSDP
                # unit (e.g. MTP `eh_proj`) crash in dgrad with a null-A
                # cuBLAS error. Still gated on `optim_grads_params` because
                # the hook is only relevant when parameters are sharded.
